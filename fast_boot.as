...
int main(uint argc, vector<var> &in argv)
{
	...
	// argv[0] tells us whether hello_world.as compiled
	bool bHelloWorldCompiled = argv[0];
	...
	markCorners(0, resolutions[0].columns,
		resolutions[0].rows);
	...
	if (bHelloWorldCompiled)
	{
		printLine(">>> Executing hello_world.as", 0,
			Display_TextForeground_Black, Display_TextBackground_Green);
	}
	else
	{
		printLine("!!! hello_world.as compilation failed", 0,
			Display_TextForeground_Yellow, Display_TextBackground_Red);
	}
	
	// -----------------------------------------------------

	// We're done with the GPU
	close(fdVideo);
